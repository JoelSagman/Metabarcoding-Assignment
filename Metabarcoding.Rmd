...
[Joel Sagman](https://github.com/JoelSagman/Metabarcoding-Assignment)
...
row.names(PDat) <- FlDat$Quadrate  # adds row names pertaining to each quadrate to the species data
```
...
```{r}
ggtree(P_tree, layout = "rectangular") %<+% FlDat + # builds tree
...
PlDat<-data.frame(NMDS1=NMDSdat$points[,1], #Prepares data for plotting
                 ...
                 SampleID=row.names(FlDat),
                 Population = as.character(FlDat$Population),
                 Location = FlDat$Location)
```
